...
public final class PerkEffectPresentation {
    ...
    private static String statDescription(StatId stat, double value) {
        return switch (stat) {
            ...
            case WOODCUTTING_LOG_EXPERIENCE -> "vanilkové zkušenosti za pokácený log +" + number(value);
            case WOODCUTTING_LEAF_EXPERIENCE -> "vanilkové zkušenosti za list +" + number(value);
            case GOLDEN_LEAF_APPLE_CHANCE -> "šance na zlaté jablko z listí " + signedPercentagePoints(value);
            case SAPLING_GROWTH_MULTIPLIER -> "rychlost růstu zasazených saplingů " + signedPercent(value);
            case FOOD_SATURATION_MULTIPLIER -> "sytost z jídla " + signedPercent(value);
            ...
            case FOOD_COOKING_SPEED -> "rychlost přípravy jídla " + signedPercent(value);
            case TNT_POWER -> "síla řízeného odstřelu " + signedPercent(value);
            case MINING_BLOCK_EXPERIENCE -> "vanilkové zkušenosti za vytěžený blok +" + number(value);
            case DRILLING_SPEED_MULTIPLIER -> "rychlost Drillingu " + signedPercent(value - 1.0);
            case DRILLING_COOLDOWN_REDUCTION -> "zkrácení cooldownu Drillingu " + signedPercent(value);
            case TNT_BONUS_DROP_CHANCE -> "šance na bonusový drop z řízeného TNT " + signedPercentagePoints(value);
            case DIGGING_SPEED -> "rychlost kopání lopatou " + signedPercent(value);
            case DIGGING_BLOCK_EXPERIENCE -> "vanilkové zkušenosti za vykopaný blok +" + number(value);
            case FARMING_BONUS_DROP_CHANCE -> "šance na dodatečný výtěžek sklizně " + signedPercentagePoints(value);
            case FARMING_HARVEST_EXPERIENCE -> "vanilkové zkušenosti za sklizeň +" + number(value);
            ...
            case ANIMAL_GROWTH_MULTIPLIER -> "rychlost růstu zvířat " + signedPercent(value);
            case ANIMAL_BONUS_DROP_CHANCE -> "šance na dodatečný výtěžek ze zvířat " + signedPercentagePoints(value);
            case BREEDING_EXPERIENCE_MULTIPLIER -> "vanilkové zkušenosti z rozmnožování " + signedPercent(value);
    ...
    private static String mechanicDescription(MechanicId mechanic) {
        return switch (mechanic) {
            ...
            case GOLDEN_LEAVES -> "vzácné zlaté jablko z listí";
            case INSTANT_LEAF_BREAK -> "okamžité ničení listí sekerou se zachováním dropů";
