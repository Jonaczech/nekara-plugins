...
The unit tests cover indicator movement and edge reflection, target boundaries, hits, misses, timeout, target bounds, state transitions, double completion prevention, and configuration fallback validation.

## Purpur 26.1.2 manual acceptance

Use a clean test server with Java 25 and only the target server plus NekaraFishing first. Give the test player the relevant permissions and place the player in a configured world.

### 1. Normal successful fishing
...
7. Verify the action bar has no `FISH` label, the target and indicator are clearly distinguishable, and the success particle effect appears briefly before delivery.
8. Verify the action bar contains only timing information and the BossBar above it fills one segment group after each successful pull.
9. Repeat enough sessions to observe different required hit counts between 3 and 5.
10. After each successful pull, verify the real bobber moves toward the player and stops before a solid wall; verify `minigame.hook-pull-distance: 0` disables that movement.

### 9. ValhallaMMO fishing difficulty scaling
...
### 8. Another plugin changes loot

1. Install a test plugin that changes the existing `CAUGHT_FISH` event's `ItemStack` or XP.
2. Complete NekaraFishing.
3. Verify NekaraFishing does not replace the `Item`, alter its metadata, add another item, or change XP.
4. Verify a canceled `CAUGHT_FISH` event does not produce a catch-success message or sound.

## Additional edge cases

Also verify off-hand duplicate interactions, changing the held item, dropping the rod, opening an inventory, death, spectator mode, creative mode, a broken rod, a missing hook, a caught entity, an already-canceled fishing event, and server shutdown.
